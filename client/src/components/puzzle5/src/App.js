...
function App() {
  ...
  useEffect(() => {
    if (config) {
      (async () => {
        ...
        _records.current = features.map((feature) => {
          return {
            ...feature.attributes,
            imageURL: imageURLs
              .filter((value) => value.objectId === feature.attributes.objectid)
              .shift().imageURL,
            solved: false,
            hintActivated: false,
            skipped: false,
        ...
        if (config.sortKeys) {
          ...
          while (keys.length) {
            ...
            const found = _records.current.find(
              (record) => record.objectid === key
            );
            if (found) {
              sorted.push(found);
            }
  ...
  useEffect(() => {
    ...
    if (
      ...
      cardBody.scrollTo({
        top: bubbleContainer.offsetHeight,
        behavior: "smooth",
      });
    } else {
  ...
  const findItemIndex = (objectid) => {
    return _records.current.findIndex(
      (element) => element.objectid === objectid
    );
  };

  const calculateScore = () => {
    const numCorrect = _records.current.filter(
      (value) => value.solved && !value.skipped
    ).length;
      ...
      {config && (
        <>
          <header className="border-bottom border-bottom-1 d-flex justify-content-between align-items-center p-3">
            <h1 className="fs-5 ms-2">Hunt Treasures City!</h1>
            <button
          ...
          >
            {!hideCongratsScreen && (
              <CongratsScreen
                ...
                onDismiss={() => dismissCongratsScreen()}
              ></CongratsScreen>
            )}
            ...
            {!hideInstructions && (
              <Instructions onDismiss={() => setHideInstructions(true)} />
            )}
            {selectedQuestion && (
              <THMap
                id="map"
                ...
                onSolve={(objectid) => markSolved(objectid)}
              ></THMap>
            )}
            ...
            {selectedQuestion && (
              ...
              >
                ...
                >
                  ...
                  >
                    <h3 className="h5">
                      QUESTION {findItemIndex(selectedQuestion.objectid) + 1} OF{" "}
                      {_records.current.length}
                    </h3>
                    {!selectedQuestion.skipped && selectedQuestion.solved && (
                      <span>Solved</span>
                    )}
                  ...
                  >
                    ...
                    <PhotoCredits
                      ...
                      sourceReferenceURL={
                        selectedQuestion.image_source_reference_page
                      }
                      license={selectedQuestion.image_license}
                      licenseReferenceURL={
                        selectedQuestion.image_license_reference_page
                      }
                      className="small p-1"
                    ></PhotoCredits>
                  </div>
                  ...
                  <div
                    className="card-body d-flex flex-column overflow-auto p-4 pb-0 border-top-2"
                    style={{
                  ...
                  >
                    ...
                    >
                      {selectedQuestion.solved && (
                        ...
                        >
                          {findItemIndex(selectedQuestion.objectid) <
                            _records.current.length - 1 &&
                            selectedQuestion.solved && (
                              <button
                                className="btn btn-sm btn-primary ms-3 w-100"
                          ...
                          {findItemIndex(selectedQuestion.objectid) ===
                            _records.current.length - 1 &&
                            _records.current.filter(
                              (question) => question.solved
                            ).length === _records.current.length && (
                              <button
                                className="btn btn-sm btn-primary ms-3 w-100"
                                style={{ maxWidth: "120px", float: "right" }}
                                onClick={() => showCongratsScreen()}
                              >
                                See your results!
                              </button>
                            )}
                          ...
                          <p
                            dangerouslySetInnerHTML={{
                              __html: selectedQuestion.exclamation,
                            }}
                          ></p>
                        </div>
                      )}
                      ...
                      >
                        ...
                        <p
                          dangerouslySetInnerHTML={{
                            __html: selectedQuestion.prompt,
                          }}
                        ></p>
                        <button
                          target="blank"
                          ...
                          onClick={() =>
                            markHintActivated(selectedQuestion.objectid)
                          }
                        >
                          Need a hint?
                        </button>

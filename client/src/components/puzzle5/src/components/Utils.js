...
const lookUpConfig = async (edition) => {
  const featureLayerRegistryURL =
    "https://services.arcgis.com/nzS0F0zdNLvs7nc8/arcgis/rest/services/survey123_aedff645769549a5bea20220e2da313f_results/FeatureServer/0";
  const response = await fetch(
    featureLayerRegistryURL +
      "/query?where=edition='" +
      edition +
  ...
  if (json.features.length) {
    ...
    const imageURLs = await getImageURLs(featureLayerRegistryURL, [
      attributes.objectid,
    ]);
    config = {
      ...
      sortKeys:
        ...
        attributes.sort_keys.split(",").map((value) => parseInt(value)),
    };
  }
  return config;
};
...
export const getImageURLs = async (serviceURL, objectIds) => {
  ...
  const json = await response.json();
  return json.attachmentGroups.map((entry) => {
...
export const getItemInfo = async (itemID) => {
  ...
  const response = await fetch(
    SHARING_URL + "rest/search?q=" + itemID + "&f=json"
  );
  const json = await response.json();

  const surveyFormItem = json.results
    .filter((value) => value.type === "Form" && value.access === "public")
    .shift();

  const featureServiceItem = json.results
    .filter(
      (value) =>
  ...
  return {
    ...
    description:
      (surveyFormItem && surveyFormItem.description) ||
      "You're subtitle here too (once you add your Treasure Hunt to the registry).",
...
export const parseArgs = () => {
  ...
  if (parts.length === 2) {
    args = parts[1].split("&").reduce(function (accumulator, value) {
      var temp = value.split("=");
      if (temp.length === 2) {
        accumulator[temp[0].toLowerCase()] = temp[1];
      }
      return accumulator;
    }, args);
  }
